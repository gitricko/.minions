...
checked-in JSON file `mnemon/seed.json`, imported by `boot.sh` on every
...
2. **Edit** `mnemon/seed.json` — append to the `insights` array with correct field names:
...
4. **Validate before commit** (the exact step `boot.sh` runs):
   `mnemon import --dry-run mnemon/seed.json`
