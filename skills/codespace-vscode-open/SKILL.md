...
~/.hermes/skills/minions/codespace-vscode-open/scripts/install.sh

# Or from the skill directory
skills/codespace-vscode-open/scripts/install.sh
```
...
# Or from skill dir
skills/codespace-vscode-open/scripts/vscode-open.sh /path/to/file.md
```
...
1. Skill in `skills/codespace-vscode-open/` (git-tracked)
2. Script symlinked to `~/.hermes/scripts/vscode-open.sh` (runtime)
3. Wiki article in `../wiki/vscode-cli-codespaces.md`
4. Mnemon entry in `mnemon/seed.json` (validated via `mnemon import --dry-run`)
